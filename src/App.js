...
const PasswordManagement = ({ navigate }) => {
    ...
    const handleSave = async (type) => {
        ...
        try {
            await updateDoc(passwordRef, { [fieldToUpdate]: newPassword });
...
const EmployeeForm = ({ employee, onFinish }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        if (photoPreview && photoPreview !== formData.photoUrl) {
            try {
                const storageRef = ref(storage, `artifacts/${appId}/public/photos/${Date.now()}_${formData.name.replace(/\s+/g, '_')}`);
        ...
        try {
            if (employee?.id) {
                await updateDoc(doc(db, `artifacts/${appId}/public/data/employees`, employee.id), employeeData);
            } else {
...
const MachineLogForm = ({ machineName, employees, onFinish }) => {
    ...
    const handleSubmit = async (e) => { 
        ...
        if (isMaintenanceLog) {
            ...
            await addDoc(collection(db, `artifacts/${appId}/public/data/maintenance_logs`), maintenanceData);
        } else {
            const logData = {
                machineName,
                logType,
            ...
            await addDoc(collection(db, `artifacts/${appId}/public/data/machine_logs`), logData); 
        }
        onFinish(); 
    };
...
const DataManagement = ({ navigate }) => {
    ...
    const handleExport = async () => {
        ...
        const stockDoc = await getDoc(doc(db, `artifacts/${appId}/public/data/stock/main`));
        if (stockDoc.exists()) {
            data.stock = [{ _id: 'main', ...stockDoc.data() }]; 
        }
...
const SaleForm = ({ stock, onFinish }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        const stockDocRef = doc(db, `artifacts/${appId}/public/data/stock/main`);
        const salesLogRef = collection(db, `artifacts/${appId}/public/data/sales_logs`);
    ...
    const handleStockUpdate = async () => {
        ...
        try {
            await runTransaction(db, async (transaction) => {
                ...
                const stockLogRef = doc(collection(db, `artifacts/${appId}/public/data/stock_logs`));
